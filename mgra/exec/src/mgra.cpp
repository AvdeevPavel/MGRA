...
void printchr(const string& outname, const set< pair<path_t,bool> >& AllChr ) {
	...
	out << " chromosome of length " << path.size();
#ifdef HAVE_BLOCK_LENGTH
	out << " (" << len << " bp)";
#endif
	out << " follows:" << endl;
    ...
    out << endl << "# Reconstructed genome " << outname << " has " << AllChr.size() << " chromosomes" << endl;
    cout << endl << "Reconstructed genome " << outname << " has " << AllChr.size() << " chromosomes" << endl;
...
int main(int argc, char* argv[]) {
    ...
    cout << "MGRA (Multiple Genome Rearrangements & Ancestors) ver. 1.0" << endl;
    cout << "(c) 2008,09 by Max Alekseyev <maxal@cse.sc.edu>" << endl;
...
///////////////////////////////////////////////////////////////////////////////
    ...
    {
	set<vertex_t> ToDel;
	get_short_blocks(LG,OB,BL,ToDel);

	while( !ToDel.empty() ) {
	    const vertex_t& x = *ToDel.begin();
            const vertex_t& y = OB[x];
	...
	cout << "Blocks left: " << S.size()/2 << endl;
    }
#endif
...
/// STAGE 3, somewhat unreliable
    ...
    */
	...
	for(int i=0;i<NC;++i) {
	    ...
	    //ofstream trx( (MBG.set2name(MBG.TColor[i]) + "_x_tr").c_str() );
