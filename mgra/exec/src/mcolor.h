...
mcolor_t MBGraph::addtree(const string& t) {
    if( t[0]=='(' ) {
	if( t[t.size()-1] != ')' ) {
	...
	for(size_t j=1;j<t.size() - 1; ++j) {
	    ...
	    case ',': if( p==0 )
		{
		    ...
		    mcolor_t Q1 = addtree( t.substr(1,j-1) );
		    mcolor_t Q2 = addtree( t.substr(j+1,t.size()-j-2) );

		    DiColor.insert( Q1 );
		    DiColor.insert( Q2 );

		    copy(Q2.begin(),Q2.end(),inserter(Q1,Q1.end()));
